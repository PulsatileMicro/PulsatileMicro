%% 546 Network Results Review
% Before running this script, what you have to do are:
% 1. Run main_546.m (in the directory with main_546.m file)
...
[Row Col]=size(list);
for i=1:Row
  % If 546.mat exist, load the data from .mat file but not .his files
  if strfind(list(i,:),'546.mat')
    load 546.mat;
    saveFlag=0;
    break;
  end
...
if saveFlag
  PeriodNum=4;    % Number of cardiac cycles in the simulation
...
for j=1:VesNum
  ...
  PI_Damp(j)=(max(PAll(1,t_plot,j))-min(PAll(1,t_plot,j)))./MeanP(j,1)-(max(PAll(3,t_plot,j))-min(PAll(3,t_plot,j)))./MeanP(j,3);
...
%   OrgPTT_U(j)=round(indu);
  ...
  % Tapering Rate
  % (Rin-Rout)/Rmean
%   TaperRate(j)=(sqrt(MeanA(j,1)/pi)-sqrt(MeanA(j,3)/pi))/sqrt(MeanA(j,2)/pi);
  TaperRate(j)=(sqrt(MeanA(j,1)/pi)-sqrt(MeanA(j,3)/pi))/(sqrt(MeanA(j,1)/pi)+sqrt(MeanA(j,2)/pi)+sqrt(MeanA(j,3)/pi))*3;
%   TaperRate(j)=(sqrt(MeanA(j,1)/pi)-sqrt(MeanA(j,3)/pi))/Len(j);
end

% Postprocessing of PI_U
% In the 546 network, some segments show invalid PI_U
% They are replaced by the mean PI_U
PI_U(PI_U<0)=-PI_U(PI_U<0);
MeanPIInd=[521 499 293 406 407 408 409 399 302 361 401];
PI_U(MeanPIInd)=mean(PI_U);

% Process PTT_U
% MeanPTTInd=[499];   % 361 302 266 265 409 407不对，499有正有负
% % OrgPTT_U(361)=OrgPTT_U(361)-24;
% OrgPTT_U(302)=OrgPTT_U(302)-20;
% OrgPTT_U(266)=OrgPTT_U(266)-14;
% OrgPTT_U(265)=OrgPTT_U(265)-12;
% OrgPTT_U(409)=OrgPTT_U(409)-14;
% OrgPTT_U(407)=OrgPTT_U(407)-12;
% OrgPTT_U(499)=mean(OrgPTT_U);
...
% PlotMorph(RPSI_U);title('RPSI_U');
% PlotMorph(RPSI_P);title('RPSI_P');
...
figure;hold on;   % 画TaperRate vs. PI damping rate
for i=1:3
...
%   end
  ...
  % Study the morphological difference among types
  Diam_Typ(i)=mean(Diam(tmpInd));
  Diam_max(i)=max(Diam(tmpInd));
  Diam_min(i)=min(Diam(tmpInd));
  R_Typ(i)=mean(R(tmpInd));
  
  % Taper Rate among different types
  Taper_Mean(i)=mean(TaperRate(tmpInd));
  Taper_Std(i)=std(TaperRate(tmpInd));
  switch i
    case 1
      plot(TaperRate(tmpInd),PI_Damp(tmpInd),'ro');
    case 2
      plot(TaperRate(tmpInd),PI_Damp(tmpInd),'ro');
    case 3
      plot(TaperRate(tmpInd),PI_Damp(tmpInd),'b^');
  end
end
figure;hold on;whitebg('w');
subplot(221)
