...
function MeanBottomRatioA=PressureProc546(dt,inFileName,outFileName,Vel,periodNum,Diam,scale_u0,scale_r0)
% Column 1: Time
% Column 2: Velocity
...
for i=1:m
  ...
  if Vel(m,3)==0
    ratio=Vel(i,1)/mean(new_artvel_all)*133;
    new_artvel_all=new_artvel_all*ratio;
  else
    ratio=Vel(i,1)/mean(new_venvel_all)*133;
  ...
  art_ivel=interp1(new_time_all, new_artvel_all, itime, 'cubic');
  ven_ivel=interp1(new_time_all, new_venvel_all, itime, 'cubic');
