...
function MeanBottomRatioA=VelProc(dt,inFileName,outFileName,artVel,venVel,periodNum,Diam,scale_u0,scale_r0)
...
% new_time_all=single(new_time_all);
% new_artvel_all=single(new_artvel_all);
% new_venvel_all=single(new_venvel_all);

% Normalize the velocity to the value from the data file
ratio=artVel/mean(new_artvel_all)*1000;
new_artvel_all=new_artvel_all*ratio;
ratio=venVel/mean(new_venvel_all)*1000;
...
art_ivel=interp1(new_time_all, new_artvel_all, itime, 'cubic')/1e6*pi*(Diam(1)*1e-6).^2/4/scale_u0/scale_r0/scale_r0;
ven_ivel=interp1(new_time_all, new_venvel_all, itime, 'cubic')/1e6*pi*(Diam(1)*1e-6).^2/4/scale_u0/scale_r0/scale_r0;
